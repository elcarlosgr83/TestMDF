...
namespace mdf::detail {
  ...
  uint64_t Ev4Block::Write ( std::streambuf& buffer ) {
    ...
    link_list_ [ kIndexRange ] = range_event_ != nullptr ? range_event_->Index () :
                                 0;

    for ( size_t index_m = 0; index_m < length_m_; ++index_m ) {
      const auto index = 5 + index_m;
      const auto *block = reinterpret_cast<const MdfBlock*> ( scope_list_ [
                            index_m ] );
  ...
  void Ev4Block::FindReferencedBlocks ( const Hd4Block &hd4 ) {
    parent_event_ = dynamic_cast<const Ev4Block*> ( hd4.Find ( Link (
                      kIndexParent ) ) );
    range_event_ = dynamic_cast<const Ev4Block*> ( hd4.Find ( Link (
                     kIndexRange ) ) );
    scope_list_.clear ();
