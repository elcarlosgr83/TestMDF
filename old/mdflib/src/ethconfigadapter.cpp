...
namespace {
  ...
  void CreateDataBytes ( mdf::IChannel& cn_frame, uint16_t byte_offset ) {
    ...
    if ( mdf::IChannel* frame_bytes = cn_frame.CreateChannelComposition (
                                        name.str () );
         frame_bytes != nullptr ) {
...
namespace mdf {
  ...
  void EthConfigAdapter::CreateConfig ( IDataGroup& dg_block ) {
    ...
    if ( IChannelGroup* cg_checksum_error =
           dg_block.CreateChannelGroup ( "ETH_ChecksumError" );
    ...
    if ( IChannelGroup* cg_length_error =
           dg_block.CreateChannelGroup ( "ETH_LengthError" );
    ...
    if ( IChannelGroup* cg_receive_error =
           dg_block.CreateChannelGroup ( "ETH_ReceiveError" );
  ...
  void EthConfigAdapter::CreateFrameChannels ( IChannelGroup& group ) const {
    ...
    if ( IChannel* nof_rec = CreateBitsChannel ( *cn_frame,
                             "ETH_Frame.ReceivedDataByteCount", 23, 0, 16 );
         nof_rec != nullptr ) {
      nof_rec->Unit ( "B" );
    }

    if ( IChannel* frame_length = CreateBitsChannel ( *cn_frame,
                                  "ETH_Frame.DataLength", 25, 0, 16 );
    ...
    if ( IChannel* padding_bytes = CreateBitsChannel ( *cn_frame,
                                   "ETH_Frame.PadByteCount", 32, 0, 16 );
  ...
  const {
    ...
    if ( IChannel* frame_length = CreateBitsChannel ( *cn_frame,
                                  "ETH_ChecksumError.DataLength", 23, 0, 16 );
         frame_length != nullptr ) {
      frame_length->Unit ( "B" );
    }
    ...
    if ( IChannel* nof_rec = CreateBitsChannel ( *cn_frame,
                             "ETH_ChecksumError.ReceivedDataByteCount", 33, 0, 16 );
         nof_rec != nullptr ) {
      nof_rec->Unit ( "B" );
    }
    ...
    if ( IChannel* padding_bytes = CreateBitsChannel ( *cn_frame,
                                   "ETH_ChecksumError.PadByteCount", 36, 0, 16 );
         padding_bytes != nullptr ) {
      padding_bytes->Unit ( "B" );
    }
  ...
  const {
    ...
    if ( IChannel* nof_rec = CreateBitsChannel ( *cn_frame,
                             "ETH_LengthError.ReceivedDataByteCount", 23, 0, 16 );
         nof_rec != nullptr ) {
      nof_rec->Unit ( "B" );
    }
    ...
    if ( IChannel* frame_length = CreateBitsChannel ( *cn_frame,
                                  "ETH_LengthError.DataLength", 25, 0, 16 );
         frame_length != nullptr ) {
      frame_length->Unit ( "B" );
    }
    ...
    if ( IChannel* padding_bytes = CreateBitsChannel ( *cn_frame,
                                   "ETH_LengthError.PadByteCount", 32, 0, 16 );
         padding_bytes != nullptr ) {
      padding_bytes->Unit ( "B" );
    }
  ...
  const {
    ...
    if ( IChannel* nof_rec = CreateBitsChannel ( *cn_frame,
                             "ETH_ReceiveError.ReceivedDataByteCount", 23, 0, 16 );
         nof_rec != nullptr ) {
      nof_rec->Unit ( "B" );
    }
    ...
    if ( IChannel* frame_length = CreateBitsChannel ( *cn_frame,
                                  "ETH_ReceiveError.DataLength", 25, 0, 16 );
         frame_length != nullptr ) {
      frame_length->Unit ( "B" );
    }
    ...
    if ( IChannel* padding_bytes = CreateBitsChannel ( *cn_frame,
                                   "ETH_ReceiveError.PadByteCount", 32, 0, 16 );
         padding_bytes != nullptr ) {
      padding_bytes->Unit ( "B" );
    }
