...
namespace mdf {
  ...
  ISourceInformation* IConfigAdapter::CreateSourceInformation (
    ...
    if ( !protocol_.empty () || message_id_ != 0 || bus_channel_ != 0
      ...
      if ( message_length_ != 0 ) {
        MdProperty message_length ( "MessageLength",
                                    std::to_string ( message_length_ ) );
  ...
  IChannel* IConfigAdapter::CreateBitsChannel ( IChannel& parent,
    ...
    IChannel* frame_bits = CreateBitChannel ( parent, name, byte_offset,
                           bit_offset );
