...
namespace mdf::detail {
  ...
  public:
    ...
    uint64_t ReadDataRecord ( std::streambuf& buffer,
    ...
    [ [ nodiscard ] ] uint64_t WriteVlsdSample ( std::streambuf& buffer,
      uint8_t record_id_size,
      const std::vector<uint8_t>& source );

    [ [ nodiscard ] ] uint64_t WriteCompressedVlsdSample ( std::vector<uint8_t>&
      dest,
      uint8_t record_id_size,
      const std::vector<uint8_t>& buffer );
