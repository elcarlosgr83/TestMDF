    ...
    if ( data_list_.empty () || block_size == 0 ) {
      ...
      if ( IsRdBlock () ) {
        ...
        static_cast<size_t> ( mean_index + block_size ) <= data_list_.size () ) {
          std::copy_n ( data_list_.cbegin () + mean_index, block_size,
          record_buffer.begin () );
      }

      value.MeanValid = channel.GetChannelValue ( record_buffer, value.MeanValue,
      array_index );

      if ( const auto min_index = static_cast<int> ( ( sample * 3 * block_size ) +
      block_size );
      static_cast<size_t> ( min_index + block_size ) <= data_list_.size () ) {
        std::copy_n ( data_list_.cbegin () + min_index, block_size,
        record_buffer.begin () );
        }

        value.MinValid = channel.GetChannelValue ( record_buffer, value.MinValue,
        array_index );

        if ( const auto max_index = static_cast<int> ( ( sample * 3 * block_size ) +
        ( 2 * block_size ) );
        static_cast<size_t> ( max_index + block_size ) <= data_list_.size () ) {
          std::copy_n ( data_list_.cbegin () + max_index, block_size,
          record_buffer.begin () );
      }

      value.MaxValid = channel.GetChannelValue ( record_buffer, value.MaxValue,
      array_index );
      } else {
        ...
        if ( const auto mean_data_index = static_cast<int> ( sample * 3 *
        block_data_size );
        static_cast<size_t> ( mean_data_index + block_data_size ) <=
        data_list_.size () ) {
          std::copy_n ( data_list_.cbegin () + mean_data_index, block_data_size,
          record_buffer.begin () );
      }

      if ( const auto mean_invalid_index = static_cast<int> ( invalid_index +
      ( sample * 3 * block_invalid_size ) );
      static_cast<size_t> ( mean_invalid_index + block_invalid_size ) <=
      data_list_.size () ) {
        ...
        if ( const auto min_data_index = static_cast<int> ( ( sample * 3 *
        block_data_size ) + block_data_size );
        static_cast<size_t> ( min_data_index + block_data_size ) <=
        data_list_.size () ) {
      ...
      static_cast<int> ( invalid_index + ( sample * 3 * block_invalid_size ) +
      block_invalid_size );
      static_cast<size_t> ( min_invalid_index + block_invalid_size ) <=
      data_list_.size () ) {
        ...
        static_cast<size_t> ( max_data_index + block_data_size ) <=
        data_list_.size () ) {
      ...
      static_cast<int> ( invalid_index + ( sample * 3 * block_invalid_size ) +
      ( 2 * block_invalid_size ) );
      static_cast<size_t> ( max_invalid_index + block_invalid_size ) <=
      data_list_.size () ) {
