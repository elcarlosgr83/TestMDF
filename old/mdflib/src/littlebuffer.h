...
namespace mdf {
  ...
  template <typename T> LittleBuffer<T>::LittleBuffer ( const T& value ) {
    if ( constexpr int num = 1;
    * ( char* ) &num == 0 ) { // Computer uses big endian
  ...
  template <typename T> LittleBuffer<T>::LittleBuffer ( const
