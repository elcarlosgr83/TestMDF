...
namespace mdf::detail {
  ...
  void Dg3Block::GetBlockProperty ( BlockPropertyList &dest ) const {
    ...
    dest.emplace_back ( "Record ID size [bytes]",
                        std::to_string ( nof_record_id_ ) );
  }
  ...
  void Dg3Block::ParseDataRecords ( std::streambuf& buffer,
                                    uint64_t nof_data_bytes ) {
    if ( nof_data_bytes == 0 ) {
      return;
    }
  ...
  void Dg3Block::ParseRangeDataRecords ( std::streambuf& buffer,
                                         uint64_t nof_data_bytes,
                                         DgRange& range ) {
    if ( nof_data_bytes == 0 ) {
      return;
