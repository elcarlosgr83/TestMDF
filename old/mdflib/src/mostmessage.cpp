...
namespace mdf {
  ...
  void MostEthernetPacket::ToRaw ( SampleRecord& sample ) const {
    ...
    MdfHelper::UnsignedToRaw ( true, 0, 16, SpecifiedNofBytes (),
                               record.data () + 22 );
    MdfHelper::UnsignedToRaw ( true, 0, 16, ReceivedNofBytes (),
                               record.data () + 24 );
    MdfHelper::UnsignedToRaw ( true, 0, 16, DataBytes ().size (),
                               record.data () + 26 );
  ...
  void MostAllocTable::ToRaw ( SampleRecord& sample ) const {
    ...
    MdfHelper::UnsignedToRaw ( true, 0, 16,
                               static_cast<uint16_t> ( TableData ().size () ),
                               record.data () + 12 );
