...
// Use ghc::filesystem for all builds
...
namespace mdf::detail {
  ...
  */
  ...
  std::string MdfBlock::ReadTx3 ( std::streambuf& buffer,
                                  size_t index_tx ) const {
    if ( Link ( index_tx ) > 0 ) {
  ...
  std::string MdfBlock::ReadTx4 ( std::streambuf& buffer,
                                  size_t index_tx ) const {
    if ( Link ( index_tx ) > 0 ) {
      SetFilePosition ( buffer, Link ( index_tx ) );
      Tx4Block tx;
  ...
  */
  ...
  void MdfBlock::UpdateLink ( std::streambuf& buffer, size_t link_index,
                              int64_t link ) {
