...
namespace mdf::detail {
  ...
  void Ca4Block::GetBlockProperty ( BlockPropertyList &dest ) const {
    ...
    const auto nof_data_blocks = Storage () == ArrayStorage::DgTemplate ?
                                 cycle_count_list_.size () : 0;
    const auto nof_dynamic_sizes = ( Flags () & CaFlag::DynamicSize ) != 0 ? 3 *
                                   dimensions_ : 0;
    const auto nof_input_quantities = ( Flags () & CaFlag::InputQuantity ) != 0 ?
                                      3 * dimensions_ : 0;
    const auto nof_output_quantities = ( Flags () & CaFlag::OutputQuantity ) != 0 ?
                                       3 * dimensions_ : 0;
    const auto nof_comp_quantities = ( Flags () & CaFlag::ComparisonQuantity ) != 0
                                     ? 3 : 0;
    const auto nof_axis_conversions = ( Flags () & CaFlag::Axis ) != 0 ?
                                      dimensions_ : 0;
    const auto nof_axis = ( ( Flags () & CaFlag::Axis ) != 0 )
                          && ( ( Flags () & CaFlag::FixedAxis ) == 0 ) ? 3 * dimensions_ : 0;

    dest.emplace_back ( "Links", "", "", BlockItemType::HeaderItem );

    size_t max_index;
    const auto* hd_block =
      HeaderBlock (); // Need header block so the function Find() search from there.

    for ( size_t link_index = 0; link_index < nof_links
          && hd_block != nullptr; ++link_index ) {
      std::ostringstream desc;
    ...
    for ( size_t axis = 0; axis < axis_value_list_.size (); ++axis ) {
      std::ostringstream label;
      label << "Axis " << axis << ":";
      dest.emplace_back ( label.str (),
                          std::to_string ( axis_value_list_ [ axis ] ) );
    }
    ...
    for ( size_t cycle = 0; cycle < cycle_count_list_.size (); ++cycle ) {
      ...
      dest.emplace_back ( label.str (),
                          std::to_string ( cycle_count_list_ [ cycle ] ) );
    }
  }
