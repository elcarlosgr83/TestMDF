...
namespace mdf {
  ...
  void LinConfigAdapter::CreateFrameChannels ( IChannelGroup& group ) const {
    ...
    if ( IChannel* count = CreateBitsChannel ( *cn_frame,
                           "LIN_Frame.ReceivedDataByteCount", 10, 0, 4 );
         count != nullptr ) {
      count->Unit ( "B" );
    }

    if ( IChannel* length = CreateBitsChannel ( *cn_frame, "LIN_Frame.DataLength",
                            10, 4, 4 );
         length != nullptr ) {
      length->Unit ( "B" );
    }
    ...
    if ( IChannel* crc_model = CreateBitsChannel ( *cn_frame,
                               "LIN_Frame.ChecksumModel", 20, 0, 2 );
         crc_model != nullptr ) {
      crc_model->DataType ( ChannelDataType::SignedIntegerLe );
    }

    if ( IChannel* sof = CreateBitsChannel ( *cn_frame, "LIN_Frame.SOF", 21, 0,
                         64 );
         sof != nullptr ) {
      sof->Unit ( "ns" );
    }
    ...
    if ( IChannel* break_length = CreateBitsChannel ( *cn_frame,
                                  "LIN_Frame.BreakLength", 37, 0, 32 );
         break_length != nullptr ) {
      break_length->Unit ( "ns" );
    }

    if ( IChannel* break_length = CreateBitsChannel ( *cn_frame,
                                  "LIN_Frame.BreakDelimiterLength", 41, 0, 32 );
         break_length != nullptr ) {
      break_length->Unit ( "ns" );
    }
  }
  ...
  const {
    ...
    if ( IChannel* count = CreateBitsChannel ( *cn_frame,
                           "LIN_ChecksumError.ReceivedDataByteCount", 10, 0, 4 );
         count != nullptr ) {
      count->Unit ( "B" );
    }

    if ( IChannel* length = CreateBitsChannel ( *cn_frame,
                            "LIN_ChecksumError.DataLength", 10, 4, 4 );
         length != nullptr ) {
      length->Unit ( "B" );
    }
    ...
    if ( IChannel* crc_model = CreateBitsChannel ( *cn_frame,
                               "LIN_ChecksumError.ChecksumModel", 20, 0, 2 );
         crc_model != nullptr ) {
      crc_model->DataType ( ChannelDataType::SignedIntegerLe );
    }

    if ( IChannel* sof = CreateBitsChannel ( *cn_frame, "LIN_ChecksumError..SOF",
                         21, 0, 64 );
         sof != nullptr ) {
      sof->Unit ( "ns" );
    }
    ...
    if ( IChannel* resp =
           CreateBitsChannel ( *cn_frame, "LIN_ChecksumError.ResponseBaudrate", 33, 0,
                               32 );
         resp != nullptr ) {
      resp->DataType ( ChannelDataType::FloatLe );
      resp->Unit ( "bits/s" );
    ...
    if ( IChannel* break_length = CreateBitsChannel ( *cn_frame,
                                  "LIN_ChecksumError.BreakLength", 37, 0, 32 );
         break_length != nullptr ) {
      break_length->Unit ( "ns" );
    }

    if ( IChannel* break_length = CreateBitsChannel ( *cn_frame,
                                  "LIN_ChecksumError.BreakDelimiterLength", 41, 0, 32 );
         break_length != nullptr ) {
      break_length->Unit ( "ns" );
    }
  ...
  void LinConfigAdapter::CreateReceiveErrorChannels ( IChannelGroup& group )
  const {
    ...
    if ( IChannel* nof_spec = CreateBitsChannel ( *cn_frame,
                              "LIN_ReceiveError.SpecifiedDataByteCount", 10, 4, 4 );
         nof_spec != nullptr ) {
      nof_spec->Unit ( "B" );
    }

    if ( IChannel* nof_rec = CreateBitsChannel ( *cn_frame,
                             "LIN_ReceiveError.ReceivedDataByteCount", 10, 0, 4 );
         nof_rec != nullptr ) {
      nof_rec->Unit ( "B" );
    }

    if ( IChannel* frame_length = CreateBitsChannel ( *cn_frame,
                                  "LIN_ReceiveError.DataLength", 11, 0, 4 );
    ...
    if ( IChannel* crc_model = CreateBitsChannel ( *cn_frame,
                               "LIN_ReceiveError.ChecksumModel", 9, 6, 2 );
         crc_model != nullptr ) {
      crc_model->DataType ( ChannelDataType::SignedIntegerLe );
    }

    if ( IChannel* sof = CreateBitsChannel ( *cn_frame, "LIN_ReceiveError.SOF", 20,
                         0, 64 );
         sof != nullptr ) {
      sof->Unit ( "ns" );
    }

    if ( IChannel* baudrate = CreateBitsChannel ( *cn_frame,
                              "LIN_ReceiveError.Baudrate", 28, 0, 32 );
         baudrate != nullptr ) {
      baudrate->DataType ( ChannelDataType::FloatLe );
      baudrate->Unit ( "bits/s" );
    }

    if ( IChannel* resp = CreateBitsChannel ( *cn_frame,
                          "LIN_ReceiveError.ResponseBaudrate", 32, 0, 32 );
         resp != nullptr ) {
      resp->DataType ( ChannelDataType::FloatLe );
    ...
    if ( IChannel* break_length = CreateBitsChannel ( *cn_frame,
                                  "LIN_ReceiveError.BreakLength", 36, 0, 32 );
         break_length != nullptr ) {
      break_length->Unit ( "ns" );
    }

    if ( IChannel* delimiter = CreateBitsChannel ( *cn_frame,
                               "LIN_ReceiveError.BreakDelimiterLength", 40, 0, 32 );
         delimiter != nullptr ) {
      delimiter->Unit ( "ns" );
    }
  }
  ...
  void LinConfigAdapter::CreateSyncChannels ( IChannelGroup& group ) const {
    ...
    if ( IChannel* sof = CreateBitsChannel ( *cn_frame, "LIN_SyncError.SOF", 9, 0,
                         64 );
         sof != nullptr ) {
      sof->Unit ( "ns" );
    }
    ...
    if ( IChannel* baudrate = CreateBitsChannel ( *cn_frame,
                              "LIN_SyncError.Baudrate", 17, 0, 32 );
         baudrate != nullptr ) {
      baudrate->DataType ( ChannelDataType::FloatLe );
      baudrate->Unit ( "bits/s" );
    }
    ...
    if ( IChannel* break_length = CreateBitsChannel ( *cn_frame,
                                  "LIN_SyncError.BreakLength", 21, 0, 32 );
         break_length != nullptr ) {
      break_length->Unit ( "ns" );
    }

    if ( IChannel* del_break_length = CreateBitsChannel ( *cn_frame,
                                      "LIN_SyncError.BreakDelimiterLength", 25, 0, 32 );
  ...
  void LinConfigAdapter::CreateWakeUpChannels ( IChannelGroup& group ) const {
    ...
    if ( IChannel* baudrate = CreateBitsChannel ( *cn_frame, "LIN_WakeUp.Baudrate",
                              9, 0, 32 );
         baudrate != nullptr ) {
      baudrate->DataType ( ChannelDataType::FloatLe );
      baudrate->Unit ( "bits/s" );
    ...
    if ( IChannel* sof = CreateBitsChannel ( *cn_frame, "LIN_WakeUp.SOF", 13, 0,
                         8 );
         sof != nullptr ) {
      sof->Unit ( "ns" );
    }
  ...
  void LinConfigAdapter::CreateTransmissionErrorChannels (
    IChannelGroup& group ) const {
    const bool mandatory = writer_.MandatoryMembersOnly ();
    IChannel* cn_frame = group.CreateChannel ( "LIN_TransmissionError" );

    if ( cn_frame == nullptr ) {
      MDF_ERROR () << "Failed to create the channel. Name: " <<
    ...
    if ( IChannel* specified = CreateBitsChannel ( *cn_frame,
                               "LIN_TransmissionError.SpecifiedDataByteCount", 10, 0, 4 );
         specified != nullptr ) {
      specified->Unit ( "B" );
    }

    if ( IChannel* crc_model = CreateBitsChannel ( *cn_frame,
                               "LIN_TransmissionError.ChecksumModel", 10, 4, 2 );
         crc_model != nullptr ) {
      crc_model->DataType ( ChannelDataType::SignedIntegerLe );
    }

    if ( IChannel* sof = CreateBitsChannel ( *cn_frame, "LIN_TransmissionError.SOF",
                         11, 0, 64 );
         sof != nullptr ) {
      sof->Unit ( "ns" );
    }
    ...
    if ( IChannel* baudrate = CreateBitsChannel ( *cn_frame,
                              "LIN_TransmissionError.Baudrate", 19, 0, 32 );
         baudrate != nullptr ) {
      baudrate->DataType ( ChannelDataType::FloatLe );
      baudrate->Unit ( "bits/s" );
    }
    ...
    if ( IChannel* break_length = CreateBitsChannel ( *cn_frame,
                                  "LIN_TransmissionError.BreakLength", 23, 0, 32 );
         break_length != nullptr ) {
      break_length->Unit ( "ns" );
    }

    if ( IChannel* delimiter = CreateBitsChannel ( *cn_frame,
                               "LIN_TransmissionError.BreakDelimiterLength", 27, 0, 32 );
         delimiter != nullptr ) {
      delimiter->Unit ( "ns" );
    }
  }
  ...
  void LinConfigAdapter::CreateSpikeChannels ( IChannelGroup& group ) const {
    ...
    if ( IChannel* baudrate = CreateBitsChannel ( *cn_frame, "LIN_Spike.Baudrate",
                              9, 0, 32 );
         baudrate != nullptr ) {
      baudrate->DataType ( ChannelDataType::FloatLe );
      baudrate->Unit ( "bits/s" );
  ...
  const {
    ...
    if ( IChannel* baudrate = CreateBitsChannel ( *cn_frame, "LIN_LongDom.Baudrate",
                              9, 0, 32 );
         baudrate != nullptr ) {
      baudrate->DataType ( ChannelDataType::FloatLe );
      baudrate->Unit ( "bits/s" );
    ...
    if ( IChannel* sof = CreateBitsChannel ( *cn_frame, "LIN_LongDom.SOF", 13, 0,
                         64 );
         sof != nullptr ) {
      sof->Unit ( "ns" );
    }

    if ( IChannel* length = CreateBitsChannel ( *cn_frame, "LIN_LongDom.Length", 21,
                            0, 32 );
         length != nullptr ) {
      length->Unit ( "ns" );
    }
