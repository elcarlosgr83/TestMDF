...
namespace mdf {
  ...
  void IDataGroup::DetachSampleObserver ( const ISampleObserver *observer )
  const {
  ...
  bool IDataGroup::IsEmpty () const {
    ...
    return std::all_of ( cg_list.cbegin (),
    cg_list.cend (), [] ( const auto * group ) {
  ...
  IChannelGroup* IDataGroup::GetChannelGroup ( const std::string_view &name )
  const {
    if ( name.empty () ) {
      return nullptr;
    }
  ...
  bool IDataGroup::IsSubscribingOnChannel ( const IChannel& channel ) const {
    ...
    [ & ] ( const auto & observer ) {
      ...
      const auto* channel_observer = dynamic_cast<const IChannelObserver*>
                                     ( observer );
