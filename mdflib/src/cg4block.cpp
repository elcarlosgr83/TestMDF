...
namespace mdf::detail {
  ...
  const IChannel* Cg4Block::GetXChannel ( const IChannel &reference ) const {
    ...
    // this group
    if ( x_axis_list.size () == 3 && x_axis_list [ 1 ] == Index ()
         && x_axis_list [ 2 ] ) {
  ...
  void Cg4Block::GetBlockProperty ( BlockPropertyList &dest ) const {
    ...
    dest.emplace_back ( "Name TX", ToHexString ( Link ( kIndexName ) ),
                        acquisition_name_,
  ...
  uint64_t Cg4Block::WriteVlsdSample ( std::streambuf& buffer,
                                       uint8_t record_id_size,
                                       const std::vector<uint8_t>& source ) {
  ...
  const IDataGroup* Cg4Block::DataGroup () const {
    ...
    return mdf_block != nullptr ? dynamic_cast<const IDataGroup*>
           ( mdf_block ) : nullptr;
  }
