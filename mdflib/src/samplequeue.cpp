...
namespace mdf::detail {
  ...
  void SampleQueue::CleanQueue ( std::unique_lock<std::mutex>& lock ) {
    ...
    while ( !IsEmpty () ) {
      ...
      writer_.file_->sputn ( reinterpret_cast<const char*>
                             ( sample.record_buffer.data () ),
  ...
  void SampleQueue::IncrementNofSamples ( uint64_t record_id ) const {
    ...
    list.cend (), [ & ] ( IChannelGroup * group ) -> void {
  ...
  void SampleQueue::SaveFlexRayMessage ( const IChannelGroup& group,
                                         uint64_t time,
