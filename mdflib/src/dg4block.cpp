...
namespace mdf::detail {
  ...
  void Dg4Block::GetBlockProperty ( BlockPropertyList& dest ) const {
    ...
    dest.emplace_back ( "Link Data", ToHexString ( Link ( kIndexData ) ),
                        "Link to Data",
  ...
  void Dg4Block::ParseDataRecords ( std::streambuf& buffer,
                                    uint64_t nof_data_bytes ) const {
    if ( nof_data_bytes == 0 ) {
      return;
    }
  ...
  uint64_t Dg4Block::ReadRecordId ( std::streambuf& buffer,
  ...
  bool Dg4Block::FinalizeCgAndVlsdBlocks ( std::streambuf& buffer, bool update_cg,
    ...
    while ( count < dt_block->DataSize () ) {
      ...
      if ( ( cg_block->Flags () & CgFlag::VlsdChannel ) != 0 ) {
        ...
        const size_t record_size = cg_block->NofDataBytes () +
                                   cg_block->NofInvalidBytes ();
