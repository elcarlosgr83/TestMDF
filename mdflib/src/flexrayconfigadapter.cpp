...
namespace mdf {
  ...
  void FlexRayConfigAdapter::CreateFlxFrameHeader ( IDataGroup& data_group )
  const {
    const IChannel* cn_data_byte = nullptr;
  ...
  void FlexRayConfigAdapter::CreateFlxErrorFrame ( IDataGroup& data_group )
  const {
    const IChannel* cn_data_byte = nullptr;
  ...
  void FlexRayConfigAdapter::CreateFrameChannel ( IChannelGroup& channel_group )
  const {
    ...
    if ( IChannel* payload = CreateSubChannel ( *parent_frame, "PayloadLength", 12,
                             0, 8 );
         payload != nullptr ) {
      CnUnit unit ( "word" );
    ...
    if ( IChannel* data_length = CreateSubChannel ( *parent_frame, "DataLength", 13,
                                 0, 8 );
         data_length != nullptr ) {
      CnUnit unit ( "B" );
    ...
    if ( IChannel* duration = CreateSubChannel ( *parent_frame, "FrameLength", 20,
                              0, 32 );
         duration != nullptr ) {
      duration->Unit ( "ns" );
    }
  ...
  void FlexRayConfigAdapter::CreatePduChannel ( IChannelGroup& channel_group )
  const {
    const bool mandatory_members_only = MandatoryMembersOnly ();
  ...
  void FlexRayConfigAdapter::CreateFrameHeaderChannel ( IChannelGroup&
      channel_group ) const {
    const bool mandatory_members_only = MandatoryMembersOnly ();
    ...
    if ( IChannel* data_length = CreateSubChannel ( *parent_frame, "FillDataLength",
                                 13, 0, 8 );
         data_length != nullptr ) {
      CnUnit unit ( "B" );
      data_length->SetCnUnit ( unit );
  ...
  void FlexRayConfigAdapter::CreateNullFrameChannel ( IChannelGroup&
      channel_group ) const {
    const bool mandatory_members_only = MandatoryMembersOnly ();
  ...
  void FlexRayConfigAdapter::CreateErrorFrameChannel ( IChannelGroup&
      channel_group ) const {
    const bool mandatory_members_only = MandatoryMembersOnly ();
    ...
    if ( IChannel* duration = CreateSubChannel ( *parent_frame, "FrameLength", 21,
                              0, 32 );
         duration != nullptr ) {
      duration->Unit ( "ns" );
    }
  ...
  void FlexRayConfigAdapter::CreateSymbolChannel ( IChannelGroup& channel_group )
  const {
    const bool mandatory_members_only = MandatoryMembersOnly ();
    ...
    if ( IChannel* length = CreateSubChannel ( *parent_frame, "SymbolLength", 10, 0,
                            8 );
         length != nullptr ) {
      CnUnit unit ( "MT" );
  ...
  void FlexRayConfigAdapter::CreateFlxChannelMaskChannel (
    IChannel& parent_channel,
    uint32_t byte_offset,
    uint16_t bit_offset ) const {
    std::ostringstream name;
  ...
  void FlexRayConfigAdapter::CreateFillDataBytesChannel ( IChannel&
      parent_channel,
      uint32_t byte_offset ) const {
    std::ostringstream name;
