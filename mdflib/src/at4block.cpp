...
namespace {
  ...
  bool CopyBytes ( std::streambuf& source, std::streambuf& dest,
                   uint64_t nof_bytes ) {
    uint8_t temp = 0;
  ...
  bool FileToBuffer ( const std::string& filename, mdf::ByteArray& dest ) {
    try {
      ...
      if ( size > 0 ) {
        ...
        if ( file != nullptr ) {
          const auto nof_bytes = fread ( dest.data (), 1, static_cast<size_t> ( size ),
                                         file );
...
namespace mdf::detail {
  ...
  void At4Block::GetBlockProperty ( BlockPropertyList& dest ) const {
    ...
    dest.emplace_back ( "Mime Type TX", ToHexString ( Link ( kIndexType ) ),
                        file_type_,
  ...
  void At4Block::ReadData ( std::streambuf& buffer,
    ...
    if ( IsEmbedded () ) {
      const bool error = IsCompressed () ? !Inflate ( buffer, dest_buffer,
                         nof_bytes_ )
