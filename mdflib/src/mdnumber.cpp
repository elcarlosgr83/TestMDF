...
namespace mdf {
  ...
  MdNumber::MdNumber ( uint64_t number, MdDataType data_type )
    ...
    if ( data_type_ == MdDataType::MdHex ) {
      char temp [ 30 ] = {};
      std::snprintf ( temp, sizeof ( temp ), "0x%llX",
  ...
  uint64_t MdNumber::BitMask () const {
    return bit_mask_.has_value () ? bit_mask_.value () :
  ...
  void MdNumber::ToXml ( IXmlNode& root_node,
    ...
    if ( bit_mask_.has_value () ) {
      char temp [ 30 ] = {};
      std::snprintf ( temp, sizeof ( temp ), "0x%llX",
                      static_cast<unsigned long long> ( bit_mask_.value () ) );
