...
namespace mdf {
  ...
  std::vector<uint8_t> MdfHelper::NsToCanOpenDateArray ( uint64_t
      ns_since_1970 ) {
  ...
  std::vector<uint8_t> MdfHelper::NsToCanOpenTimeArray ( uint64_t
      ns_since_1970 ) {
  ...
  uint64_t MdfHelper::CanOpenTimeArrayToNs ( const std::vector<uint8_t>&
      buffer ) {
  ...
  uint64_t MdfHelper::CanOpenDateArrayToNs ( const std::vector<uint8_t>&
      buffer ) {
    ...
    const uint8_t day_in_week = ( buffer [ 4 ] & 0xE0 ) >>
                                5; // 1 Monday..7 = Sunday
  ...
  std::string MdfHelper::FormatDouble ( double value, uint8_t decimals,
                                        bool fixed,
