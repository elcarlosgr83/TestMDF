...
namespace mdf::detail {
  ...
  */
  ...
  [ [ nodiscard ] ] std::string ReadBlockType ( std::streambuf& buffer );
    uint64_t ReadByte ( std::streambuf& buffer, std::vector<uint8_t>& dest,
    uint64_t size );
    uint64_t WriteByte ( std::streambuf& buffer,
    ...
    uint64_t WriteStr ( std::streambuf& buffer, const std::string &source,
    uint64_t size );
  ...
  public:
    ...
    protected:
      ...
      uint64_t ReadLinks3 ( std::streambuf& buffer,
      size_t nof_links ); ///< Reads MDF3 links into the link list.
      uint64_t ReadHeader4 ( std::streambuf&
      buffer ); ///< Read in MDF4 header and links.
      ...
      void WriteTx4 ( std::streambuf& buffer, size_t index_tx,
  ...
  */
  ...
  uint64_t MdfBlock::ReadNumber ( std::streambuf& buffer, T &dest ) const {
    ...
    template <typename T>
    uint64_t MdfBlock::WriteNumber ( std::streambuf& buffer,
