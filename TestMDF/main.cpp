...
void write ( const std::string &filePath ) {
  ...
  for ( int i = 0; i < 3; ++i ) {
    uint64_t t_ns = measurementStartNs + static_cast<uint64_t>
                    ( i + 1 ) * sampleStepNs;
...

    {
      ...
      const uint8_t torque_raw = encodeTorqueRaw ( torque_eng, kTorqueMin, kTorqueMax,
                                 kTorqueOffset, kTorqueFactor );
      payload [ 2 ] = torque_raw; // bitStart=16 → byte 2
    }

    {
      const double speed_eng = 800 + i * 200;  // engineering value (rpm)
      const uint16_t speed_raw = encodeSpeedRaw ( speed_eng, kSpeedMin, kSpeedMax,
                                 kSpeedOffset, kSpeedFactor );
...
void read ( const std::string &filePath ) {
  ...
  for ( auto& dg : dataGroups ) {
    ...
    std::cout << "Processing DataGroup with" << cg_list.size () << "ChannelGroups"
              << std::endl;
      ...
      if ( cg.GetName () == "CAN_DataFrame" ) {
        ...
        for ( int64_t sample = 0; sample < maxSamples; ++sample ) {
          ...
          if ( messageId == 2297431040U && payload.size () >= 5 ) {
            const uint64_t timestamp_ns = static_cast<uint64_t> ( std::llround (
                                            relativeTimeSeconds * 1e9 ) );
            const uint8_t torque_raw = payload [ 2 ];
            const double torque = static_cast<double> ( torque_raw ) - 125.0;
            const uint16_t speed_raw = static_cast<uint16_t> ( payload [ 3 ] |
                                       ( payload [ 4 ] << 8 ) );
